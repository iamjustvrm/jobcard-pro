...
export default function AdminDashboard() {
  ...
  const calculateMargin = (buy, sell) => {
      ...
      if (s === 0) return { profit: 0, percent: 0 };
      const profit = s - b;
      const percent = Math.round((profit / s) * 100);
  ...
  const handleBulkParse = () => {
      ...
      alert(`🚀 Genius Ingest: ${count} items processed & auto-tagged!`);
      setBulkText('');
  };
            ...
            <div className="space-y-6 animate-in fade-in">
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                    <div className={`p-6 rounded-xl border relative overflow-hidden group ${theme.card}`}>
                        ...
                        <div className="flex justify-between items-end border-t border-slate-700 pt-2"><div><div className="text-[10px] text-blue-400 font-bold uppercase">Labor (Profit)</div><div className="text-lg font-bold text-white">₹{financials.totalLabor.toLocaleString()}</div></div><div className="text-right"><div className="text-[10px] text-orange-400 font-bold uppercase">Parts (Cost)</div><div className="text-lg font-bold text-white">₹{financials.totalParts.toLocaleString()}</div></div></div>
                    </div>
                    <div className={`p-6 rounded-xl border relative ${theme.card}`}><h3 className={`text-[10px] font-bold uppercase tracking-widest text-slate-400 mb-2`}>Avg Ticket Quality</h3><p className={`text-4xl font-black ${theme.textMain}`}>₹{sciData.avgTicket.toLocaleString()}</p><p className="text-xs text-slate-500 mt-2">Per Job Average</p><div className={`absolute top-6 right-6 text-2xl ${sciData.avgTicket > 5000 ? 'text-green-500' : 'text-slate-600'}`}>{sciData.avgTicket > 5000 ? '💎' : '📉'}</div></div>
                    <div className={`p-6 rounded-xl border relative ${theme.card}`}><h3 className={`text-[10px] font-bold uppercase tracking-widest text-slate-400 mb-2`}>Job Volume</h3><div className="flex items-end gap-2"><p className={`text-4xl font-black ${theme.textMain}`}>{jobs.length}</p><span className="text-sm font-bold text-slate-500 mb-1">Total Jobs</span></div><div className="mt-3 flex gap-2"><div className="text-center px-3 py-1 bg-green-500/10 rounded border border-green-500/20"><div className="text-xl font-bold text-green-400">{sciData.ready}</div><div className="text-[8px] uppercase text-slate-500">Ready</div></div><div className="text-center px-3 py-1 bg-slate-800 rounded border border-slate-700"><div className="text-xl font-bold text-slate-400">{jobs.length - sciData.activeCount}</div><div className="text-[8px] uppercase text-slate-500">Done</div></div></div></div>
                    <div className={`p-6 rounded-xl border border-l-4 ${sciData.pausedTotal > 0 ? 'border-l-red-500' : 'border-l-slate-700'} ${theme.card}`}><h3 className={`text-[10px] font-bold uppercase tracking-widest text-slate-400 mb-4`}>Active Bottlenecks</h3><div className="space-y-3"><div className="flex justify-between items-center"><div className="flex items-center gap-2"><div className="w-2 h-2 rounded-full bg-blue-500 animate-pulse"></div><span className="text-xs font-bold text-blue-400">Working</span></div><span className="font-mono font-bold text-white">{sciData.working}</span></div><div className="flex justify-between items-center"><div className="flex items-center gap-2"><div className="w-2 h-2 rounded-full bg-yellow-500"></div><span className="text-xs font-bold text-yellow-400">Estimating</span></div><span className="font-mono font-bold text-white">{sciData.estimate}</span></div><div className="flex justify-between items-center"><div className="flex items-center gap-2"><div className={`w-2 h-2 rounded-full ${sciData.pausedTotal > 0 ? 'bg-red-500 animate-ping' : 'bg-slate-600'}`}></div><span className={`text-xs font-bold ${sciData.pausedTotal > 0 ? 'text-red-400' : 'text-slate-500'}`}>Paused</span></div><span className={`font-mono font-bold ${sciData.pausedTotal > 0 ? 'text-red-400' : 'text-slate-500'}`}>{sciData.pausedTotal}</span></div></div></div>
                </div>
                {/* 2. SCIENTIFIC ROW */}
                <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                ...
                {/* 4. REVENUE RECOVERY */}
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    <div className={`lg:col-span-2 p-6 rounded-xl border ${theme.card}`}><h3 className="text-xs font-bold uppercase text-blue-400 mb-4 tracking-widest">💸 Revenue Recovery</h3><div className="grid grid-cols-1 md:grid-cols-2 gap-4 max-h-64 overflow-y-auto pr-1">{lists.pendingEstimates.map(job => (<div key={job.id} className="p-3 border border-slate-700 rounded bg-slate-800/30 flex justify-between items-center"><div><div className={`font-bold ${theme.textMain}`}>{job.customerName}</div><div className="text-xs text-slate-500">{job.regNo}</div></div><button onClick={() => window.open(`https://wa.me/91${job.customerPhone}?text=Estimate Reminder...`, '_blank')} className="bg-green-600 text-white px-3 py-1.5 rounded text-[10px] font-bold">WHATSAPP</button></div>))}</div></div>
                    <div className={`p-6 rounded-xl border border-orange-900/20 ${theme.card}`}><h3 className="text-xs font-bold uppercase text-orange-400 mb-4 tracking-widest">📉 Low Stock Radar</h3><div className="space-y-2 max-h-64 overflow-y-auto pr-1">{lists.lowStock.map(item => (<div key={item.id} className="flex justify-between items-center text-sm border-b border-slate-700 pb-1"><span className={theme.textMain}>{item.name}</span><span className="font-bold text-red-500">{item.stock} left</span></div>))}</div></div>
                </div>
            </div>
        )}
